...
VALORES_POR_FAIXA = {
    ...
    # Valores para as demais faixas populacionais
}
...
def metric_card(title, value, delta=None, prefix="R$"):
    ...
    <div class="metric-container">
        ...
        <h2 style="font-size:1.8rem; margin:0">{prefix} {value:,.2f}</h2>
    """
    if delta:
        ...
        <p style="font-size:0.9rem; margin:0; color:{delta_color}">
            {delta_arrow} {abs(delta):,.2f} ({abs(delta/value*100 if value else 0):.1f}%)
        </p>
        """
    html += "</div>"
...
def display_summary():
    ...
    with col1:
        st.download_button(
            ...
            file_name=f"orcamento_mais_gestor_{datetime.datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv"
        )
...
st.markdown(
    ...
    <div style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-bottom: 20px;">
        <p style="margin: 0; text-align: right;">Data: {datetime.datetime.now().strftime('%d/%m/%Y')}</p>
    </div>
    """,
    unsafe_allow_html=True
)
...
# Abas para navegação
tab1, tab2, tab3, tab4 = st.tabs(["📊 P1 - Consultoria", "💻 P2 - Tecnologia", "🔄 P3 - Integração", "📝 Resumo"])
